...
export class SearchService {
  private allPages: SearchResult[] = [
    ...
    },
    {
      ...
      class="lucide lucide-cable-icon lucide-cable w-[clamp(1.75rem,6vw,2.25rem)] h-[clamp(1.75rem,6vw,2.25rem)]"
    >
  ...
  search(query: string): SearchResult[] {
    ...
    this.store.getStudents().forEach((student: any) => {
      ...
      if (match) {
        const desc = `Student • ${student.grade ?? '—'} • ${
          student.email ?? ''
        }`;
    ...
    this.store.getTeachers().forEach((teacher: any) => {
      ...
      if (match) {
        const desc = `Teacher • ${teacher.subject ?? '—'} • ${
          teacher.email ?? ''
        }`;
        const r: SearchResult = {
          type: 'teacher',
    ...
    const handleConnections = (connections: Connection[]) => {
      ...
      connections.forEach((c) => {
        const sid = (c.studentId ?? (c as any).student_id) as
          | number
          | undefined;
        const tid = (c.teacherId ?? (c as any).teacher_id) as
          | number
          | undefined;
    ...
    if (Array.isArray(maybe)) {
      ...
      (maybe as Observable<Connection[]>)
        .pipe(take(1))
        .subscribe(handleConnections);
    }
